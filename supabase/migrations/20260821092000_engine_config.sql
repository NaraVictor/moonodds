-- ---------------------------------------------------------------------------
-- Kicka, the engine configuration
--
-- Moved out of supabase/seed.sql, which has been deleted along with the rest of
-- the sample data. This was the one part of that file that was never sample
-- data: without an `ai_engine_config` row with status 'active', runDailyPicks
-- returns {skipped: "no active engine config"} and the product generates
-- nothing, forever, with no error.
--
-- It had never reached the deployed database at all. `supabase db push` applies
-- migrations and does not run seeds, so the remote carried the full schema and
-- no configuration: every scheduled daily-picks run would have skipped even
-- with a working API plan behind it. Being a migration is what fixes that.
--
-- The tipster row comes along because predictions reference it.
--
-- PENALTIES ARE PERCENTS. The Convex-era config stored them as fractions, so
-- redCardCarryoverPenalty 0.05 meant 5%. Under the v2.2 table that reads as
-- 0.05%, which is arithmetically valid and functionally absent;
-- validateEngineVariables flags it and the Office surfaces it.
--
-- Keys match src/lib/engine/variables.ts exactly. A key the table does not
-- recognise is silently ignored and the built-in default runs instead, so a
-- near-miss name is worse than an absent one.
--
-- The prompt block between the ENGINE PROMPT markers is generated by
-- `pnpm engine:sync` from src/lib/engine/prompt.ts. Edit the TypeScript source,
-- never this file; `pnpm engine:check` fails the build if the two drift.
-- ---------------------------------------------------------------------------
