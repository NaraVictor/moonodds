-- ============================================================================
-- Kicka, TEST FIXTURE for the security suite. NOT application data.
--
-- This was supabase/seed.sql, which `supabase start` and `supabase db reset`
-- ran automatically, so every database came up pre-loaded with 60 invented
-- predictions, five demo accounts and a fake payment history. Seeding is now
-- off in config.toml and this file is applied by nothing except
-- `pnpm verify:security`.
--
-- It still exists because the twenty paywall checks are assertions about
-- ACCESS, and access is meaningless with nothing to be denied: "a locked-out
-- user gets 0 picks but a true total" cannot be proved against an empty board.
-- The five accounts are the five access tiers, and each check needs its tier
-- to exist. Deleting this would not make the suite simpler, it would make it
-- vacuous, and a paywall test that passes because there is nothing behind the
-- paywall is worse than no test.
--
-- The engine configuration that used to live here moved to
-- 20260821092000_engine_config.sql. That part was never sample data: without
-- it, runDailyPicks skips every run.
--
-- NEVER apply this to a deployed database. It creates accounts with a known
-- password and marks one of them super-admin.
-- ============================================================================

-- Original seed header follows.
--
