...
-- AI engine config
--
-- Keys match src/lib/engine/variables.ts exactly. That matters: a key the table
-- does not recognise is silently ignored and the built-in default runs instead,
-- so a near-miss name is worse than an absent one.
--
-- PENALTIES ARE PERCENTS. The Convex-era config stored them as fractions,
-- redCardCarryoverPenalty 0.05 meaning 5%. Under the v2.2 table that reads as
-- 0.05%, which is arithmetically valid and functionally nothing. The values
-- below are the converted equivalents; `validateEngineVariables` flags any that
-- drift back.
--
-- The prompt itself is loaded separately, from engine_prompt.sql, which is
-- generated from src/lib/engine/prompt.ts by `pnpm engine:sync`.
-- ---------------------------------------------------------------------------
...
insert into ai_engine_config (
  ...
  jsonb_build_object(
    'xgWeight', 0.15, 'formWeight', 0.20, 'h2hWeight', 0.10,
    'homeAdvantageWeight', 0.10, 'shotsOnTargetWeight', 0.15,
    'lineupWeight', 0.10, 'keyManWeight', 0.10, 'marketEfficiencyWeight', 0.05,
    'oppositionQualityWeight', 0.05
  ),
  jsonb_build_object(
  ...
  jsonb_build_object(
    -- 7.0 rather than the old 8.5: the anchoring bands calibrate from 5.0, and
    -- publishing only above 8.5 discarded most of what the engine reasons about.
    'primarySlipFloor', 7.0, 'absoluteMinimumFloor', 5.0,
    'stakingUnit5Threshold', 9.0, 'stakingUnit4Threshold', 8.0,
    'stakingUnit3Threshold', 7.0, 'stakingUnit2Threshold', 6.0,
    'stakingUnit1Threshold', 5.0,
    'anchorTier1Score', 9.0, 'anchorTier1ConditionsRequired', 5,
    'anchorTier1ConditionsTotal', 7, 'anchorTier1CapIfUnmet', 8.5,
    'anchorTier2Score', 8.0, 'anchorTier2CapIfUnmet', 7.5,
    'anchorTier3Score', 7.0, 'anchorTier3CapIfUnmet', 6.5,
    'anchorDefaultRangeMin', 5.0, 'anchorDefaultRangeMax', 6.4,
    'varianceTieBandScore', 0.3
  ),
  jsonb_build_object(
    'chaosFilterWinlessGames', 8, 'redCardCarryoverPenalty', 15,
    'capitulationBufferPct', 12.5, 'standardBufferPct', 7,
    'clvMovementThresholdPct', 5, 'clvPenalty', 10,
    'travelDistanceThreshold', 360, 'travelPenaltyPct', 10,
    'restGameCount', 3, 'restDayWindow', 8, 'restPenaltyPct', 5,
    'artificialTurfBoost', 5, 'motivationGapBoostPct', 5,
    'mraOverperformThresholdPct', 30,
    'tier1Penalty', 20, 'tier1MitigatedPenalty', 5, 'tier1MitigationRate', 0.8,
    'tier2Penalty', 10, 'tier3GKPenalty', 8,
    'suspendedStarterPenaltyPct', 3,
    'returnFromInjuryPenaltyPct', 4, 'returnFromInjuryTier1PenaltyPct', 6,
    'positionalCascadePenaltyPct', 5, 'positionalCascadeAltBoostPct', 6,
    'squadDepthThreshold', 4, 'squadDepthPenaltyPct', 5,
    'squadCrisisThreshold', 6, 'squadCrisisPenaltyPct', 10,
    'cumulativePenaltyCapPct', 15, 'globalPenaltyCapPct', 35,
    'formDivergenceResultsThreshold', 2,
    'qualityFormWinTopHalf', 1.5, 'qualityFormWinBottomHalf', 0.75,
    'qualityFormDrawTopHalf', 0.75, 'qualityFormDrawBottomHalf', 0.4,
    'qualityFormLossTopHalf', 0.5, 'qualityFormLossBottomHalf', 1.5,
    'qualityFormDivergenceThresholdPct', 15,
    'h2hRecencyWeight1', 3.0, 'h2hRecencyWeight2', 2.5, 'h2hRecencyWeight3', 2.0,
    'h2hRecencyWeight4', 1.5, 'h2hRecencyWeight5', 1.0, 'h2hRecencyWeightRest', 0.5,
    'venueH2hLowSampleGames', 3, 'venueH2hLowSampleReductionPct', 50,
    'venueH2hBlendPct', 30, 'overallH2hBlendPct', 70
  ),
  jsonb_build_object(
    'lowOddsThreshold', 1.25,
    'lowOddsPivotMarket', 'over_under_1_5', 'lowOddsPivotValue', 'over',
    'chaosPivotMarket', 'over_under_1_5', 'chaosPivotValue', 'over',
    -- v2.1 pivoted humidity to corners; corners cannot be graded here, so the
    -- same directional call is expressed on a market that settles.
    'humidityThreshold', 60,
    'humidityPivotMarket', 'over_under_2_5', 'humidityPivotValue', 'under',
    'precipitationPenalty', 15,
    'windThresholdKmh', 40, 'windOver25PenaltyPct', 10, 'windSetPiecePenaltyPct', 8,
    'extremeWindThresholdKmh', 60, 'extremeWindExtraPenaltyPct', 10,
    'altitudeThresholdMetres', 1800, 'altitudeAwayPenaltyPct', 8,
    'altitudeHomeBoostPct', 10, 'altitudeUnacclimatizedPenaltyPct', 5,
    'heatThresholdCelsius', 32, 'heatPressPenaltyPct', 8, 'heatOver25PenaltyPct', 6,
    'coldThresholdCelsius', 2, 'coldOver25PenaltyPct', 5, 'coldSetPieceBoostPct', 5,
    'refCardHeavyYellowThreshold', 4.5, 'refCardHeavyCardsBoostPct', 10,
    'refLenientYellowThreshold', 2.5, 'refLenientCardsPenaltyPct', 8,
    'refFoulHeavyThreshold', 28, 'refFoulHeavyBoostPct', 6
  ),
  jsonb_build_object(
...
-- ENGINE PROMPT START, generated by `pnpm engine:sync` from
-- src/lib/engine/prompt.ts (prompt version 2.2, 103 variables).
